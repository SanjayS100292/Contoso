...
if (isset($_POST['submit'])) {
    ...
    $qry = "SELECT * FROM `tbllogin` WHERE `username`='$uname'";
    ...
    if ($row['password'] == $password) {
        ...
        if ($row['usertype'] == 'Admin') {
            ...
            $sql = "INSERT INTO login_logout (user_id, action, status,logintime) VALUES ('$uname', 'login', 'active',(SELECT SYSDATE()))";
            $rx = mysqli_query($con, $sql);
            echo "<script>alert('Login Successful');</script>";
            echo "<script>window.location.href='custHome.php';</script>";
        }else if ($row['usertype'] == 'Hotel') {
            $sql = "INSERT INTO login_logout (user_id, action, status,logintime) VALUES ('$uname', 'login', 'active',(SELECT SYSDATE()))";
            $rx = mysqli_query($con, $sql);
            echo "<script>alert('Login Successful');</script>";
            echo "<script>window.location.href='hotelHome.php';</script>";
        }
    } else {
